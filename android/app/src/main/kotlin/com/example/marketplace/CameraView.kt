...
//    private var depth: ((ShortArray) -> Unit)? = null
    ...
    init {
        ...
        Log.d("ARCore", "frame: ${frameLayout.width}, ${frameLayout.height} / surface: ${surfaceView.width}, ${surfaceView.height}")
...
//                backgroundRenderer.suppressTimestampZeroRendering(false);
    ...
    private val cameraCaptureCallback = object : CameraCaptureSession.CaptureCallback() {
        ...
        override fun onCaptureFailed(session: CameraCaptureSession, request: CaptureRequest, failure: CaptureFailure) {
            Log.e("ARCore", "onCaptureFailed: " + failure.frameNumber + " " + failure.reason)
        }
...
//        filter.targetFps = EnumSet.of(CameraConfig.TargetFps.TARGET_FPS_30)
    ...
    fun openCamera() {
        ...
        cpuImageReader = ImageReader.newInstance(previewSize.width, previewSize.height, ImageFormat.JPEG, 2)
...
//                            val (x, y) = getDepthCoordinate(frame, depthImage, i + 100, j + 100)
...
//    }
    ...
    private fun getDepthCoordinate(frame: Frame, depthImage: Image, cpuCoordinateX: Int, cpuCoordinateY: Int): Pair<Int, Int> {
...
//        Log.d("ARCore", "Array[${byteIndex / 2}]: ${array[byteIndex / 2]}")
    ...
    private val imageSaverCallback = object : ImageSaver.Callback {
...
//        Log.d("ARCore", "onImageAvailable")
        if (isTakePic) {
            isTakePic = false
            backgroundHandler!!.post(ImageSaver(imageReader.acquireNextImage(), captureFile, imageSaverCallback))
        }
...
//                    result.success(depthData)
            ...
            call.method.equals("getCameraPose") -> {
                getCameraPose(result)
    ...
    private fun takePicture(result: MethodChannel.Result) {
        ...
        previewCaptureRequestBuilder = cameraDevice!!.createCaptureRequest(CameraDevice.TEMPLATE_STILL_CAPTURE)
        previewCaptureRequestBuilder.addTarget(cpuImageReader!!.surface)
        captureSession!!.capture(previewCaptureRequestBuilder.build(), cameraCaptureCallback, backgroundHandler)

        isTakePic = true
    }
    private fun getCameraPose(result: MethodChannel.Result) {
...
//        }
        ...
        if(isTakePose){
            flutterPoseResult.success(serializePose(camera.displayOrientedPose))
            isTakePose=false
        }
